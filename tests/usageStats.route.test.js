...
describe('Usage Stats Admin Routes', () => {
  ...
  it('keeps OpenAI total input visible in api key usage records with cache creation', async () => {
    const handler = getApiKeyUsageRecordsHandler()
    const req = {
      params: { keyId: 'key-1' },
      query: {}
  ...
  it('keeps OpenAI total input visible in account usage records with cache creation', async () => {
    const handler = getAccountUsageRecordsHandler()
    const req = {
