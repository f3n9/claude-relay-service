...
class AccountNameCacheService {
  ...
  async refresh() {
    ...
    try {
      ...
      const ccrAccounts = results[12].status === 'fulfilled' ? results[12].value : []
      const groups = results[13].status === 'fulfilled' ? results[13].value : []

      // Bedrock 返回格式特殊处理
      const bedrockAccounts = Array.isArray(bedrockResult)
        ? bedrockResult
        : bedrockResult.accounts || []
      // GCP Vertex 返回格式特殊处理（{ success, data }）
      const gcpVertexAccounts = Array.isArray(gcpVertexResult)
        ? gcpVertexResult
        : gcpVertexResult?.data || []
